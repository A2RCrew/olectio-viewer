...
const recalculate = async (state: State): Promise<Partial<State>> => {
  ...
  return new Promise<Partial<State>>((resolve): void => {
    const { olectioContainerNode, olectioViewerNode, contentPlaceholderNode } = state;

    const containerRect = olectioContainerNode!.getBoundingClientRect();
    ...
    if (state.layout === LayoutTypes.Flow) {
      ...
      if (state.scrollMode === 'vertical') {
        ...
        const marginTop =
          parseInt(window.getComputedStyle(olectioViewerNode!).marginTop, 10) || 0;
